...
export function UpgradeBanner({
  ...
  return (
    <div className={cn("mx-auto flex items-center justify-center py-2", className)}>
      <AnimatePresence>
        <motion.div
          className="relative"
        ...
        >
          <motion.div
            ...
            animate={isHovered ? { x: -10, y: -10, opacity: 1, rotate: 360 } : { x: 0, y: 0, opacity: 0 }}
            transition={{ duration: 0.3 }}
            className="pointer-events-none absolute left-[4px] top-[2px]"
          >
            <SettingsFilled className="text-orange-500" />
          </motion.div>
          <motion.div
            initial={{ x: 0, y: 0, opacity: 0 }}
            animate={isHovered ? { x: 10, y: 10, opacity: 1, rotate: 360 } : { x: 0, y: 0, opacity: 0 }}
          ...
          <div className="relative flex h-[38px] items-center gap-2 rounded-lg border-2 border-orange-500/30 bg-orange-50/80 dark:bg-orange-950/20 backdrop-blur-sm pl-3 pr-2 text-sm shadow-sm">
            <span
            ...
            <span className="text-[0.8125rem] text-orange-700 dark:text-orange-300">
              {description}
            </span>
            {onClose && (
              <button 
                onClick={onClose}
                className="m-0 flex h-6 !w-6 shrink-0 cursor-pointer items-center justify-center rounded border-0 bg-transparent p-0 text-orange-600 hover:bg-orange-100 dark:hover:bg-orange-900/50 transition-colors"
